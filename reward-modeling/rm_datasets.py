...
class SFTDataset(Dataset):
    def __init__(self, data, tokenizer):
        ...
        max_length = min(1024,
                         max([len(tokenizer.encode(ele["prompt"] + "\n\n" + ele["response"] + '<|endoftext|>')) for ele
                              in data]))
...
class RankedDataset(Dataset):
    def __init__(self, rankings, tokenizer, max_length, order=["0", "1", "2", "3", "4"], max_num=-1):
        ...
        for i, ranks in enumerate(tqdm(rankings)):
            ...
            for response in ranks.values():
                long = long or len(tokenizer("<|endoftext|>" + prompt + "<|endoftext|>" + response + "<|endoftext|>",
                                             return_tensors="pt")["input_ids"]) > max_length
            if long:
                continue
            # Reject data with num tokens > max_length
            ranking_input_ids = []
            ...
            for model in order:
                ...
                encodings_dict = tokenizer("<|endoftext|>" + prompt + "<|endoftext|>" + response + "<|endoftext|>",
...
class RankedEvalDataset(Dataset):
    def __init__(self, rankings, tokenizer, max_length, order=["0", "1", "2", "3", "4"], max_num=-1):
        ...
        for i, ranks in enumerate(tqdm(rankings)):
            ...
            for response in ranks.values():
                long = long or len(
                    tokenizer("<|endoftext|>" + prompt + "<|endoftext|>" + response + "<|endoftext|>",
                              return_tensors="pt")["input_ids"]) > max_length
            if long:
                continue
            # Reject data with num tokens > max_length
